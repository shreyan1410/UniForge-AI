import {
  useState
} from "react";

import axios from "axios";

import DashboardLayout from "../layouts/DashboardLayout";

import {
  UploadCloud,
  Sparkles,
  FileText
} from "lucide-react";

const NotesSummarizerPage = () => {

  const [file, setFile] = useState(null);

  const [summary, setSummary] = useState("");

  const [loading, setLoading] = useState(false);

  const summarizeNotes = async () => {

    if (!file) {

      alert("Please upload notes");

      return;

    }

    try {

      setLoading(true);

      const formData = new FormData();

      formData.append("notes", file);

      const { data } = await axios.post(

        "http://localhost:5000/api/ai/summarize-notes",

        formData

      );

      setSummary(data.summary);

      setLoading(false);

    } catch (error) {

      console.log(error);

      setLoading(false);

      alert("Summarization failed");

    }

  };

  return (

    <DashboardLayout>

      {/* HERO */}

      <div className="bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-3xl p-8 text-white">

        <p className="text-sm opacity-80">

          Learning Ecosystem

        </p>

        <h1 className="text-4xl font-bold mt-2">

          AI Notes Summarizer

        </h1>

        <p className="mt-4 text-sm opacity-90 max-w-2xl leading-7">

          Upload study materials and generate AI-powered summaries,
          key concepts,
          important exam topics,
          and revision notes instantly.

        </p>

      </div>

      {/* MAIN */}

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mt-8">

        {/* LEFT */}

        <div className="bg-white rounded-3xl border border-slate-200 p-6">

          <div className="flex items-center gap-3 mb-6">

            <div className="w-12 h-12 rounded-2xl bg-violet-100 flex items-center justify-center">

              <UploadCloud className="text-violet-600" />

            </div>

            <div>

              <h2 className="text-lg font-semibold text-slate-800">

                Upload Notes

              </h2>

              <p className="text-sm text-slate-500">

                PDF Notes / PPT

              </p>

            </div>

          </div>

          <label className="border-2 border-dashed border-slate-300 rounded-3xl h-64 flex flex-col items-center justify-center cursor-pointer hover:border-violet-500 transition">

            <UploadCloud
              size={40}
              className="text-slate-400"
            />

            <p className="text-sm text-slate-500 mt-4">

              Click to upload notes

            </p>

            {

              file && (

                <div className="mt-4 flex items-center gap-2 bg-violet-50 px-4 py-2 rounded-xl">

                  <FileText
                    size={18}
                    className="text-violet-600"
                  />

                  <span className="text-sm text-violet-700">

                    {file.name}

                  </span>

                </div>

              )

            }

            <input
              type="file"
              hidden
              accept=".pdf"
              onChange={(e) =>
                setFile(e.target.files[0])
              }
            />

          </label>

          <button
            onClick={summarizeNotes}
            className="w-full mt-6 bg-violet-600 hover:bg-violet-700 transition text-white py-4 rounded-2xl font-medium"
          >

            {
              loading
                ? "Generating Summary..."
                : "Summarize Notes"
            }

          </button>

        </div>

        {/* RIGHT */}

        <div className="xl:col-span-2 bg-white rounded-3xl border border-slate-200 p-6 min-h-[500px]">

          <div className="flex items-center gap-3 mb-6">

            <div className="w-12 h-12 rounded-2xl bg-pink-100 flex items-center justify-center">

              <Sparkles className="text-pink-600" />

            </div>

            <div>

              <h2 className="text-lg font-semibold text-slate-800">

                AI Summary

              </h2>

              <p className="text-sm text-slate-500">

                Smart learning insights

              </p>

            </div>

          </div>

          {

            !summary && !loading && (

              <div className="h-[350px] flex flex-col items-center justify-center text-center">

                <div className="w-24 h-24 rounded-3xl bg-slate-100 flex items-center justify-center">

                  <Sparkles
                    size={40}
                    className="text-slate-400"
                  />

                </div>

                <h2 className="text-2xl font-semibold text-slate-800 mt-6">

                  AI Learning Summary

                </h2>

                <p className="text-slate-500 mt-3 max-w-md leading-7">

                  Upload study materials and instantly generate
                  summaries,
                  important concepts,
                  revision notes,
                  and exam-focused preparation points.

                </p>

              </div>

            )

          }

          {

            loading && (

              <div className="h-[350px] flex items-center justify-center">

                <div className="text-center">

                  <div className="w-16 h-16 border-4 border-violet-200 border-t-violet-600 rounded-full animate-spin mx-auto"></div>

                  <p className="text-slate-500 mt-6">

                    AI is summarizing notes...

                  </p>

                </div>

              </div>

            )

          }

          {

            summary && !loading && (

              <div className="bg-slate-50 rounded-3xl p-6 overflow-y-auto max-h-[500px]">

                <pre className="whitespace-pre-wrap text-sm leading-8 text-slate-700 font-sans">

                  {summary}

                </pre>

              </div>

            )

          }

        </div>

      </div>

    </DashboardLayout>

  );

};

export default NotesSummarizerPage;